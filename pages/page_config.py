...
def run_ai_analysis(n_clicks, df_json, objective):
    ...
    try:
        ...
        if llm_analyzer:
            result = llm_analyzer.analyze(df_sample, summary, objective or "")

            return html.Div([
                html.H5("AI Suggestions"),
                html.Div(str(result), style={
                    'whiteSpace': 'pre-wrap',
                    'background': '#F7FAFC',
                    'padding': '10px',
                    'borderRadius': '6px'
            ...
            return dbc.Alert("LLM not initialized", color="danger")

    except Exception as e:
        return dbc.Alert(f"AI Error: {str(e)[:200]}", color="danger")